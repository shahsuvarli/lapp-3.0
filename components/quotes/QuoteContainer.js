...
import { customerWindow, selectCustomer } from "@/features/crmSlice";
  ...
  const handleDelete = async () => {
    ...
    if (confirmResult) {
      try {
        ...
        router.push(`/projects/list/${quote.project_id}`);
        router.refresh();
      ...
      <div className="flex flex-col py-8 px-5 gap-8 bg-[#f7f6f3] rounded-md box-border animate-[rise_1s_ease-in-out w-full]">
        <div className="w-full flex justify-between items-center flex-row">
          <div className="flex flex-row self-start gap-5">
            <button
              ...
              onClick={() => router.push(`/projects/list/${quote.project_id}`)}
            >
        ...
        <Formik
          ...
          initialValues={{
            ...
            customer_name:
              newCustomer?.customer_name || quote.Customer.customer_name,
            country: newCustomer?.country || quote.Customer.country,
            state: newCustomer?.state || quote.Customer.state,
            city: newCustomer?.city || quote.Customer.city,
            account_manager_id: quote.account_manager_id,
            dsm_id: quote.dsm_id,
            created_date: moment.utc(quote.created_date).format("YYYY-MM-DD"),
            created_by: `${quote.Employees_Quote_created_byToEmployees.name} ${quote.Employees_Quote_created_byToEmployees.surname}`,
            modified_date: moment.utc(quote.modified_date).format("YYYY-MM-DD"),
            modified_by: `${quote.Employees_Quote_modified_byToEmployees.name} ${quote.Employees_Quote_modified_byToEmployees.surname}`,
        ...
        >
          {({
            ...
            <Form className="w-full flex flex-col justify-center">
              <div className="w-full h-full gap-y-6 gap-x-0 flex flex-wrap justify-between relative [&>*]:gap-2 [&>*]:flex [&>*]:flex-col [&>*]:text-sm [&>*]:min-w-[230px] [&>*]:w-1/5 [&>*]:text-[#313131] [&>*]:relative [&>div>input]:h-10 [&>div>input]:p-2 [&>div>input]:box-border [&>div>input]:rounded-md [&>div>input]:border [&>div>input]:border-solid [&>div>input]:border-[#d9d9d9] [&>div>select]:h-10 [&>div>select]:p-2 [&>div>select]:box-border [&>div>select]:rounded-md [&>div>select]:border [&>div>select]:border-solid [&>div>select]:border-[#d9d9d9]">
                <div>
